...
int server_accept(snb_server_t * server)
{
    ...
    {
    	new_fd = accept(server->sockfd, (struct sockaddr *)&client_addr, &sin_size);
    	if (new_fd <= 0)
    	{
...
int server_recv(snb_server_t * server, snb_session_t * session)
{
    int ret = 0;
    int rc;
    	...
    	case SNB_RECV_STATE_BODY:
			...
			if(expect == 0)
			{
				...
				if (snb_unpack_command(msg_buf->pbuf, &command) != 0)
					goto error:
				snb_session_push_command(session, command);
			}
			return 0;
		break;
